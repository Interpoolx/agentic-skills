...
export const Route = createFileRoute('/hpanel/repos')({
  component: ReposPage,
})
...
function ReposPage() {
  ...
  function fetchRepos() {
    ...
    const url = selectedOwner
      ? `${API_URL}/api/admin/repos?ownerId=${selectedOwner}`
  ...
  const columns = useMemo<ColumnDef<Repo>[]>(() => [
    {
      accessorKey: 'name',
      header: 'Name',
      cell: ({ row }) => {
        const url = row.original.githubUrl || `https://github.com/${row.original.ownerSlug}/${row.original.slug}`;
        return (
    ...
    {
      accessorKey: 'ownerSlug',
      header: 'Owner',
      cell: ({ row }) => (
        <a
          href={`https://github.com/${row.original.ownerSlug}`}
          target="_blank"
          rel="noreferrer"
          className="font-mono text-blue-400 hover:text-blue-300 bg-blue-400/10 px-2 py-0.5 rounded text-xs transition-colors"
        >
          @{row.original.ownerSlug}
        </a>
      )
    },
    {
    ...
    },
    {
      ...
      header: 'Skills',
      cell: ({ row }) => (
        <span className="text-gray-300 text-sm">🛠️ {row.original.totalSkills || 0}</span>
      )
    },
    {
      accessorKey: 'createdAt',
      header: 'Created',
      cell: ({ row }) => <span className="text-gray-500 text-xs">{row.original.createdAt ? new Date(row.original.createdAt).toLocaleDateString() : '-'}</span>
    },
    {
      id: 'actions',
      header: 'Actions',
          ...
          <button
            onClick={() => {
              setConfirmModal({
                ...
                message: `Are you sure you want to delete ${row.original.name}? This will cascade delete associated Skills.`,
                onConfirm: () => deleteRepo(row.original.id)
              })
            }}
    ...
    <div className="p-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-400">Manage skill repositories</p>
        </div>
      ...
      <div className="flex gap-4 mb-6">
        <select
          ...
          className="px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500 min-w-[200px]"
        >
...
function RepoModal({ repo, owners, onClose, onSuccess }: { repo: Repo | null, owners: Owner[], onClose: () => void, onSuccess: () => void }) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const url = repo ? `${API_URL}/api/admin/repos/${repo.id}` : `${API_URL}/api/admin/repos`
    const method = repo ? 'PATCH' : 'POST'
    ...
    fetch(url, {
      ...
      .then(data => {
        if (data.success) {
          toast.success(repo ? 'Repository updated successfully' : 'Repository created successfully')
          onSuccess()
        } else {
          toast.error(data.error || `Failed to ${repo ? 'update' : 'create'} repository`)
        }
      })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-gray-800 border border-gray-700 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
        <div className="p-6 border-b border-gray-700 flex justify-between items-center">
          <h2 className="text-xl font-bold text-white">{repo ? 'Edit Repository' : 'Create New Repository'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            >
              {submitting ? (repo ? 'Updating...' : 'Creating...') : (repo ? 'Update Repository' : 'Create Repository')}
            </button>
          </div>
        </form>
      </div>
