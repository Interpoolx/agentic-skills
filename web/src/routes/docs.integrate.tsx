...
function IntegrateSkills() {
  ...
  // Global skills
  '~/.${BRANDING.brand_lower_name}/skills',
  '~/.claude/skills',
]`}</CodeBlock>
...
skill's SKILL.md file to get detailed instructions.`}</CodeBlock>
  ...
  return content.replace(/^---[\\s\\S]*?---\\n/, '')
      ...
      <p>
        {BRANDING.brand_name} supports 15+ AI coding clients. Each client has its preferred
        skill directory:
      </p>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/5">
              <th className="text-left py-4 pr-6 text-[10px] font-bold uppercase tracking-widest text-gray-500">Client</th>
              <th className="text-left py-4 pr-6 text-[10px] font-bold uppercase tracking-widest text-gray-500">Local Path</th>
              <th className="text-left py-4 text-[10px] font-bold uppercase tracking-widest text-gray-500">Global Path</th>
            </tr>
          </thead>
            ...
            {clients.map((client) => (
              <tr key={client.id}>
                <td className="py-5 pr-6">
                  <span className="mr-3">{client.icon}</span>
                  <span className="font-bold text-white">{client.name}</span>
                </td>
                <td className="py-5 pr-6 font-mono text-[12px] text-gray-400">{client.local}</td>
                <td className="py-5 font-mono text-[12px] text-gray-400">{client.global}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <p>
        The {BRANDING.brand_name} CLI provides a reference implementation for skill integration:
      </p>
...
npx ${BRANDING.brand_lower_name} sync --format xml

# Read skill content for activation
npx ${BRANDING.brand_lower_name} read pdf-processing`}</CodeBlock>

      <p className="mt-8">
        See the <a href="https://github.com/Interpoolx/agentic-skills" className="text-white font-bold hover:underline">
...
const clients = [
  ...
  { id: 'agent', name: 'Universal', icon: '🌐', local: '.agent/skills', global: `~/.${BRANDING.brand_lower_name}/skills` },
]
