...
const REGISTRY_API_URL = CLI_BRANDING.api_url;
const REGISTRY_FALLBACK_URL = CLI_BRANDING.fallback_url;
...
export function getSkillLocations(baseDir: string = '.'): { name: string; path: string }[] {
    return [
        ...
        { name: 'Global', path: path.resolve(os.homedir(), `.${CLI_BRANDING.brand_lower_name}`, 'skills') }
    ];
}
