...
export async function interactiveSearch(initialQuery?: string, options: SearchOptions = {}): Promise<void> {
    p.intro(pc.cyan(pc.bold(`${CLI_BRANDING.brand_lower_name} search`)));
    ...
    if (results.length === 0) {
        ...
        p.log.info(`Browse all skills: npx ${CLI_BRANDING.brand_lower_name} list --registry`);
        p.outro('');
        return;
    }
...
async function installSelectedSkill(skill: SkillDefinition, options: SearchOptions): Promise<void> {
    ...
    const scope = await p.select({
        ...
        options: [
            ...
            { value: 'global', label: `🌍 Global (~/.${CLI_BRANDING.brand_lower_name}/skills)` }
        ]
    });
...
function showGoodbye(): void {
    ...
    console.log('');
    console.log(pc.dim(`Browse skills: https://${CLI_BRANDING.brand_lower_name}.pages.dev`));
    console.log('');
}
...
export async function searchSkillsSimple(query: string, options: SearchOptions = {}): Promise<void> {
    ...
    try {
        ...
        console.log(pc.dim(`Use interactive mode for easy install: npx ${CLI_BRANDING.brand_lower_name} search`));
